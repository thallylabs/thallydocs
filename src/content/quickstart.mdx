---
title: Quickstart
description: Publish your first documentation site with Thally Cloud and check that the live site is ready to share.
---
...
5. At Docs source, recommend Create a new docs repository. Use an existing repository only when it is already a current Thally project that I explicitly selected. Needs me: have me install or approve the Thally GitHub App for the intended GitHub account. Explain that fresh repository creation requires All repositories access so the app can initialize the new repository. If I require selected-repository access, use an existing compatible Thally repository I approve and grant access to it. Do not select or modify an unrelated repository.
...
7. If the Knowledge surfaces step is available for my workspace, ask whether each qualifying Track event should also check another customer-facing repository. Treat these as independent Track destinations, not Product sources or Product Knowledge inputs. Confirm the repository, branch, root, and exact allowed paths before adding one, or skip this optional step. Each affected destination receives its own ready-for-review pull request; Track never merges it.
8. Review the site name, thally.app address, documentation repository, production branch, Product sources, and Knowledge surfaces with me. Explain that an eligible workspace previews trial features during setup and starts its one-time 14-day Cloud trial when an authorized Publish site submission is accepted, even if background publishing later fails. Select Publish site only after those values match what I approved.
...
13. Run npx --yes @thallylabs/cli@latest check and npm run build. Fix errors caused by the content changes. Start npm run dev when the environment can keep a local server running, inspect the homepage, navigation, representative pages, code blocks, images, and API reference, then stop the server.
...
If your workspace has never had a subscription, setup previews the features
and three-site allowance of its one-time Cloud trial. Browsing the wizard does
not start the trial. See [trial access and AI Credits](/guides/thally-cloud#understand-trial-access-and-ai-credits)
before publishing.

<Note type="info" title="Check workspace capacity before you publish">
  Review [managed-site capacity](/guides/thally-cloud#plan-managed-site-capacity)
  before creating another site. If the workspace is at its active-site limit,
  an owner can add recurring capacity or delete another Cloud site without
  deleting that site's GitHub repository or documentation files.
</Note>
...
For this fresh-repository path, choose **All repositories** when installing
the Thally GitHub App on the intended account. This lets the app initialize
the repository it creates. If your organization requires selected-repository
access, use an existing compatible Thally repository and grant the app access
...
If this step is available for your workspace, you can add another repository
with Markdown, MDX, or supported React content. Existing JSX and TSX files can
receive visible prose edits; executable code and component structure stay
unchanged. Track may prepare path-scoped, ready-for-review pull requests.
Each qualifying Track event gives the primary docs and every enabled
surface an independent run and no-change decision. A surface is a Track review
destination, not a Product source, Product Knowledge input, or another managed
site. You can also add it later from **Site settings → Knowledge surfaces**.
See [how Knowledge-surface fan-out works](/guides/knowledge-surfaces) before
approving its branch and path boundaries.
...
```bash
npx --yes create-thally-docs@latest my-docs --yes --install
cd my-docs
npm run dev
```
...
<CardGroup cols="2">
  <Card title="Manage your Cloud site" icon="grid-round" href="/guides/thally-cloud">
    Learn where to find deployments, Track, Knowledge, readiness, analytics, and site settings.
  </Card>
  <Card title="Manage publishing" icon="code-simple" href="/guides/managed-hosting">
    Review production publishes, pull-request previews, rollbacks, domains, and Thally updates.
  </Card>
