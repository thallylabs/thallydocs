...
| `appearance` | Reader color mode and toggle control (see below). |
| `background` | Site-wide background image and decoration (see below). |
...
| Field | Default | Purpose |
| --- | --- | --- |
| `appearance.default` | `"system"` | Initial reader color mode: `"system"`, `"light"`, or `"dark"`. |
| `appearance.showToggle` | `true` | Show the theme toggle. Set `false` to lock readers into the mode set by `default`. |
| `appearance.contentIcons` | `"accent"` | Card and tile icon treatment: `"accent"` or `"neutral"`. |
...
| Field | Default | Purpose |
| --- | --- | --- |
| `background.image` | — | Background image for light mode. Public path or HTTPS URL (PNG, JPEG, WebP). |
| `background.imageDark` | — | Background image for dark mode. Falls back to `image` when omitted. |
| `background.decoration` | `"none"` | Overlay decoration: `"none"`, `"grid"`, or `"gradient"`. |
