...
Set `THALLY_ACCESS_PASSWORD` and a signing secret in the hosting provider:

```bash
THALLY_ACCESS_PASSWORD=a-long-random-visitor-password
THALLY_ACCESS_SECRET=a-separate-high-entropy-signing-secret
```

`THALLY_ACCESS_SECRET` signs the session cookie. It is required in production —
without it, sign-in returns an error. When unset, it falls back to
`THALLY_ADMIN_SECRET`, but a dedicated value is preferred. Generate a
high-entropy random value; never reuse the visitor password.
...
Do not commit these values to `.env`, `docs.json`, or the repository. Use the
