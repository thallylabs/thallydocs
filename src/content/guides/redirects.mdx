...
When migrating from Mintlify, the `thally migrate` command automatically reads the `redirects` field from your Mintlify `docs.json` and emits equivalent entries in the Thally `docs.json`. It also generates redirects for any pages that were renamed during migration.
