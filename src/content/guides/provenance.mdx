...
<Note>
`lastUpdated` and `lastVerified` mean different things. **Updated** is when the
page changed; **verified** is when someone last confirmed it still matches the
product. A page can be verified without being edited.
</Note>
...
# Read only by `thally check --drift`; never served publicly:
sources:
  - src/networks.ts
