...
Use the local Thally MCP migrate_docs tool if connected. Otherwise run npx --yes @thallylabs/cli@latest migrate SOURCE_URL TARGET_DIRECTORY. Do not use import_docs or --into unless I explicitly ask for an in-place merge and approve the destination. Preserve the migration log and warnings.
