...
## Plan managed-site capacity

| Plan | Active managed-site allowance |
| --- | --- |
| Free | One site |
| Cloud | Three sites included; additional capacity is a recurring site add-on |
| Enterprise | Contract-specific capacity |

Some paid workspaces also have a persisted grandfathered allowance that
preserves the non-deleted sites they had when metered site pricing launched.
For Cloud, the effective limit is the larger of that workspace-specific
allowance and the three included sites plus its recurring add-ons. This means
two Cloud workspaces can have different numeric limits; do not treat an older
workspace's allowance as the standard Cloud cap.

If site creation reports that the workspace has reached its active-site
allowance, use one of these recovery paths:

1. **Add capacity.** From **Sites**, choose **Upgrade for more sites**, or open
   **Workspace settings → Billing**. An owner can upgrade from Free or choose
   **Manage subscription** to add recurring site capacity. Enterprise
   workspaces can contact billing for a contract-limit change.
2. **Delete a Cloud site.** An owner can open an existing site, go to
   **Settings**, and choose **Delete site**. After deletion completes, the site
   no longer counts toward the active-site allowance. Thally removes it from
   the Cloud workspace but does not change or delete its GitHub repository or
   documentation files.

After the allowance changes, return to **Create a site** and retry the publish.

## Understand trial access and AI Credits

A workspace with no previous subscription can start a one-time, 14-day Cloud
trial without a payment card. The site wizard previews trial capabilities,
including Track, available Knowledge surfaces, and three active managed sites.
Opening the wizard or inspecting repository paths does not start the trial.

An authorized **Publish site** submission starts the trial before background
site creation and deployment finish. A later publishing failure does not reset
the trial clock, and retrying does not extend it. A workspace with an existing
subscription cannot start another trial. After a trial expires, the workspace
uses Free capabilities unless it has a qualifying paid subscription.

The default initial allowance is 10,000 shared AI Credits per workspace.
Track uses the workspace balance across its runs and destinations; this is
not a separate allowance for each site or repository. Check your workspace's
AI Credit balance before running Track. Credits and feature access are
separate: a remaining balance does not keep paid features unlocked after the
trial expires.
...
| **Settings**         | Manage identity, languages, Git source, runtime version, branding, page chrome, domains, reader access, feedback, analytics, Knowledge surfaces, external-site credentials, and site deletion. |
...
plan includes five members; additional members are billed separately.
