...
<Steps>
  ...
  <Step title="Choose product repositories">
    Connect up to five repositories during setup if you want Track to start
    with the site. You can skip this step and configure Track later.
  </Step>
  <Step title="Review and publish">
    Confirm the repository, production branch, site address, and optional
    Track sources. Thally saves the site before it starts background setup, so
    a safe retry resumes the same site instead of creating a duplicate.
  </Step>
</Steps>
...
| **Track** | Connect product repositories and follow each documentation update from change detection to publication. |
| **Deployments** | Inspect production and preview publishes, filter history, redeploy, repair supported failures, and roll back. |
| **Settings** | Manage identity, languages, Git source, runtime version, branding, domains, reader access, feedback, analytics, external-site credentials, and site deletion. |
