...
```bash
npx --yes @thallylabs/cli@latest migrate https://github.com/example/product/tree/main/docs --platform mintlify --yes
```

Add an output directory after the URL to control where the project is created. Without one, Thally names the directory after the repository with a `-docs` suffix, so this example creates `product-docs`.
