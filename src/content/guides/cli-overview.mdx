...
internals. Run the published `@thallylabs/cli` package with `npx` from the
project root; a generated project does not install a local `thally` binary.
...
npx --yes @thallylabs/cli@latest new guides/invite-a-member --title "Invite a team member"
...
| Update Thally | `thally starter update` | Previews or applies a compatible framework update. |

## `thally` and `create-thally-docs`

Use `npx --yes @thallylabs/cli@latest` for routine work inside a project. Use
`npx --yes create-thally-docs@latest` when you need the newest scaffold before a
...
3. Run npx --yes @thallylabs/cli@latest check without a fix flag first. Group results into blocking errors, warnings that can be proven from repository state, and warnings that require editorial or product judgment.
...
5. Run npx --yes @thallylabs/cli@latest check again. When it passes, run npm run build. If a command fails, isolate whether the failure came from your edit, existing repository state, the environment, or an external service before changing more files.
