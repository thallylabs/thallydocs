...
| `THALLY_ADMIN_SECRET` | Production password admin access | Required when `THALLY_ADMIN_PASSWORD` is set in production. Generate a high-entropy random value; never reuse the admin password. |
| `THALLY_ACCESS_PASSWORD` | Shared visitor access | Requires readers to enter a password before opening the docs. |
| `THALLY_ACCESS_SECRET` | Production visitor access | Required when `THALLY_ACCESS_PASSWORD` is set in production. Falls back to `THALLY_ADMIN_SECRET` when unset, but a separate value is preferred. |
