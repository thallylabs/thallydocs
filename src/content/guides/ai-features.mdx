...
The `/api/docs/{slug}` endpoint serves different formats based on the request. For translated content, prefix the slug with the locale code: `/api/docs/{locale}/{slug}`.
...
curl https://docs.example.com/api/docs/es/guides/private-docs \
  -H "Accept: application/json"
```
...
```bash
GET /api/docs-index
GET /api/docs-index?locale=es
```

Adding `?locale=` filters the index to pages that have a published translation in that language and returns translated titles, descriptions, and keywords. Pages without a translation in the requested language are omitted. API operations are included only for the default locale.
...
When two or more locales are configured, the file also advertises locale-specific endpoints:

```
Docs-Locale-Index: /api/docs-index?locale={locale}
Docs-Locale-Search: /api/search?q={query}&locale={locale}
Docs-Locale-Page: /api/docs/{locale}/{slug}
```
