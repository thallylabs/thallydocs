...
<Card title="Real locale URLs" icon="link-simple">
  `/es/introduction`, `/fr/guides/getting-started`. Each published translation
  has its own crawlable URL.
</Card>

<Card title="AI-powered translation" icon="wrench">
  Thally Cloud generates translations using your workspace's AI Credits.
  Self-hosted sites can translate with one command. Code and MDX structure are
  validated.
</Card>

<Card title="Automatic fallbacks" icon="book-open">
  Missing translations show the original with a notice. Stale translations stay
  visible with an update notice.
</Card>
...
readers should see, and save. Confirm that Thally may use your workspace's
shared AI Credits to generate translations. You can leave the page while Thally
generates missing or stale translations, validates the complete batch, commits
the MDX files to your connected repository, and publishes the site. Thally
never overwrites team-authored translations.

English remains the source language at unprefixed URLs. The selected languages
go live only after the full batch succeeds. If generation or validation fails,
no translation files are published; fix the reported problem and select
**Refresh translations**. The Languages section shows current, stale, missing,
and human-authored coverage from your connected repository.
...
<Step title="Translate your pages locally with the CLI">

This optional, local workflow is separate from Thally Cloud's managed
translation. Run the translate command in your documentation repository to
generate translated MDX files for a locale:
...
<Note type="info" title="Local CLI key only">
  The CLI command runs in your own environment and calls Claude through the
  Anthropic API. Set `ANTHROPIC_API_KEY` locally or pass `--api-key` to that
  command. These options do not apply to Thally Cloud, which uses workspace AI
  Credits for managed translation.
</Note>

</Step>

<Step title="Review and commit local translations">

After a local CLI translation, your content directory will look like this:
...
**Stale translation**, where a generated translation's recorded source hash differs from the current source content:
...
For a managed site, select **Refresh translations** in **Site settings →
Languages**. For a local CLI workflow, run the translate command for that page:
...
The docs search dialog searches the current language and returns translated page URLs. Agents can request `/api/docs-index?locale=fr`, `/api/search?q=installation&locale=fr`, or `/api/docs/fr/guides/getting-started` for translated content. These APIs omit pages without a published translation in the requested language.
...
When asked **"Add another documentation language? (y/N):"**, enter `y` and provide your locale codes (e.g. `es,fr,de`). The `i18n` block is written to `docs.json` automatically.
