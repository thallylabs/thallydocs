...
The dashboard overrides the structural theme, the **brand accent** (and its focus ring), the logo, the favicon, and an optional **site background** color. When a custom background is set, secondary surfaces — muted, input, border, and muted-foreground — are derived from it automatically so the entire palette stays harmonized. Tokens not covered by a managed override are defined in the source files described below.
...
This overrides the accent and focus ring. A custom background color, if set separately, overrides the page canvas, sidebar, and card surfaces and derives harmonized values for muted, input, border, and muted-foreground. Tokens not covered by a managed override come from the source-file presets below. Use the dashboard for a live adjustment and the files as the versioned baseline.
