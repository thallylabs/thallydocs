...
The dashboard overrides the structural theme, colors (accent, primary, and background), the logo, and the favicon. When a custom background color is set, secondary surfaces — muted backgrounds, borders, form inputs, and secondary text — automatically harmonize with that background. Tokens not covered by a dashboard override are defined in the source files described below.
...
This overrides the accent and focus ring; it does not change backgrounds, borders, or secondary surfaces. A custom **background color** has a broader effect: muted backgrounds, borders, form inputs, and secondary text automatically harmonize with the chosen canvas so the full page stays cohesive. Tokens not covered by a color override come from the source-file presets below. Use the dashboard for a live adjustment and the files as the versioned baseline.
