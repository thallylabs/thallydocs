...
## Dark mode and appearance locking

Dark mode is handled automatically. By default, readers toggle it with the theme switch in the top bar. Your brand palette defines both light and dark variants, so both themes stay on-brand.

You can control the initial color mode and whether readers can change it by adding `appearance` settings to `docs.json`:
...
| --- | --- | --- | --- |
| `default` | `"system"`, `"light"`, or `"dark"` | `"system"` | Initial color mode for new readers. |
| `showToggle` | boolean | `true` | Show the theme toggle in the top bar. When `false`, the mode set by `default` is enforced and the toggle is hidden. |
| `contentIcons` | `"neutral"` or `"accent"` | `"accent"` | Card and tile icon treatment. |

When `showToggle` is `false`, a flash-free pre-paint script runs in the document head so the locked mode is applied before the page renders. Reader preferences stored from a previous visit are ignored while the toggle is hidden.

Thally Cloud managed sites can also override appearance settings per-field through the Cloud branding configuration.

## Background images and decoration

Add a site-wide background behind the document surface with the `background` object in `docs.json`:
...
| Key | Type | Default | Purpose |
| --- | --- | --- | --- |
| `image` | string | — | Background image for light mode. A path under `public/` or an HTTPS URL. Accepted formats: PNG, JPEG, WebP. |
| `imageDark` | string | — | Background image for dark mode. Falls back to `image` when omitted. |
| `decoration` | `"none"`, `"grid"`, or `"gradient"` | `"none"` | Overlay decoration rendered on top of the background image. |

The background image is rendered as a fixed, full-viewport layer behind the page. When a background image is active, content and sidebar surfaces become translucent to keep text legible over the imagery. Omit the `background` object entirely (or set all fields to their defaults) to leave the site surfaces unchanged.

Thally Cloud managed sites can also supply background images and decoration through the Cloud branding configuration, with managed values taking per-field precedence over the repository config.
